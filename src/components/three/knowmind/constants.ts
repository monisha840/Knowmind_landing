...
export function threadStage(progress: number): number {
  return smooth(0.22, 0.42, progress) + smooth(0.54, 0.8, progress);
}
...
export const LAYOUT = {
  ...
  widthFill: 0.62,
  /**
   * And of that, the share it gets when it stands beside a column of copy
   * rather than alone in the frame.
   */
  sideFill: 0.52,
  heightFill: 0.72,
  minScale: 0.4,
